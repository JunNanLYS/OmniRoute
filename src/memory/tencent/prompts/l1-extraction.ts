/**
 * L1 extraction prompt family — adapted from TencentDB Agent Memory (MIT).
 *
 * Upstream source:
 ...
 *   - `ConversationMessage` shape is replaced with a structurally identical
 *     type so the local prompt builder does not import upstream types.
 *   - This module returns strings only — it does NOT write files, call LLM,
 *     or depend on ai-sdk / OpenClaw adapter imports.
 *
...
export interface L1ExtractionMessage {
  id: string;
  role: string;
  content: string;
  /** Epoch ms — used to derive the ISO timestamp shown to the LLM. */
  timestamp: number;
}
...
- "L1 Prompt 应保持输出 JSON 结构稳定，优先通过调整优先级和提取规则适配新场景。"
...
/**
 * Pick the right system prompt for the active mode.
 * Mirrors upstream `getExtractMemoriesSystemPrompt`.
 */
 ...
 *
 * Mirrors upstream `formatExtractionPrompt`:
 *   - background messages are for context only (never extracted from)
 *   - new messages are the actual extraction source
 *   - each message is rendered as `[id] [role] [ISO timestamp]: content`
 *
 * Local adaptation: the upstream `ConversationMessage` type is replaced with
 * `L1ExtractionMessage` (structurally compatible — `id`, `role`, `content`,
 * `timestamp`).
 */
export function formatExtractionPrompt(params: {
  newMessages: L1ExtractionMessage[];
  backgroundMessages?: L1ExtractionMessage[];
