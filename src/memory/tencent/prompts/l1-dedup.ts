/**
 * L1 dedup / batch conflict-detection prompt family — adapted from
 * TencentDB Agent Memory (MIT).
 *
 * Upstream source:
 *   MemoryCore/src/core/prompts/l1-dedup.ts
 *     (CONFLICT_DETECTION_SYSTEM_PROMPT, WORK_CONFLICT_DETECTION_SYSTEM_PROMPT,
 *      getConflictDetectionSystemPrompt, formatBatchConflictPrompt,
 *      CandidateMatch)
 *
 * Local adaptation:
 *   - Prompt text is preserved verbatim from upstream (these are stable
 *     contracts: any drift in wording changes the LLM's behavior).
 *   - The upstream `MemoryRecord` / `ExtractedMemory` types are replaced
 *     with structurally identical local types so this module does not import
 *     upstream symbols.
 *
...
/** Minimal subset of an L1 memory used by the dedup prompt. */
export interface DedupCandidateRecord {
  id: string;
  content: string;
  type: string;
  priority: number;
...
/** A new memory bundled with its candidate pool. */
...
- merged_content：merge/update 时的最终记忆文本。store/skip 时省略。
- merged_type：merge/update 后记忆应归属的 type。根据合并后内容本质判断。
- merged_priority：merge/update 后的新优先级（0-100 整数，merge/update 时必填）。合并后信息更完整、更确定，通常应**酌情提升** priority。
- merged_timestamps：合并后的时间戳数组。收集新记忆 + 所有被合并旧记忆的时间戳，去重排序。`;

/**
 * Pick the right dedup system prompt for the active mode.
 * Mirrors upstream `getConflictDetectionSystemPrompt`.
 */
...
/**
 * Format the batch conflict-detection user prompt with a unified candidate pool.
 *
 * Mirrors upstream `formatBatchConflictPrompt`:
 *   1. Build a unified pool of all candidate records (de-duplicated across new memories)
 *   2. Render each new memory with its related candidate-id list
 *   3. Concatenate into the final prompt
 */
export function formatBatchConflictPrompt(matches: DedupCandidateMatch[]): string {
  const unifiedPool = new Map<string, DedupCandidateRecord>();
