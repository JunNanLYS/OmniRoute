/**
 * L2 scene extraction prompt family — adapted from TencentDB Agent Memory (MIT).
 *
 * Upstream source:
 *   MemoryCore/src/core/prompts/scene-extraction.ts
 *     (buildSceneSystemPrompt, buildWorkSceneSystemPrompt, getSceneSystemPrompt,
 *      buildSceneExtractionPrompt, SceneExtractionPromptParams,
 *      SceneExtractionPromptResult)
 *
 * Local adaptation:
 *   - Prompt text is preserved verbatim from upstream.
 *   - The output spec is changed: instead of asking the LLM to perform file
 *     tool calls (read/write/edit against a sandboxed `scene_blocks/` dir),
 *     this port returns structured `SceneExtractionAction` records to the
 *     caller. The OmniRoute native worker decides how to persist them. This
 *     avoids pulling the tool-calling layer and the OpenClaw host adapter.
 *   - The 1500-char file cap and `maxScenes` enforcement (≤15 by default) are
 *     preserved via the output plan.
 *
...
/** Result of L2 scene extraction — structured actions, not file writes. */
...
export interface SceneExtractionPromptParams {
  ...
  /** List of existing scene filenames (relative). */
...
**输出语言**：\`scene_blocks/*.md\` 的所有自然语言内容（文件名、章节标题、正文）使用与"New Memories List"中记忆相同的语言；META 字段名（created/updated/summary/heat）和 \`[DELETED]\` 等标记保持英文。
...
3. **遵守分级预警**：
   - 红色预警（≥ ${maxScenes}）：**必须先通过 MERGE 减少文件数量**，将最相似的 2-4 个场景合并为 1 个，**并删除被合并的旧文件**，直到文件再调 < ${maxScenes} 后，再处理新记忆
...
请你参考这个模板输出 .md 文件的内容或基于已有md进行更新，每个md控制在1500字符内。
...
## 核心叙事
[一段连贯描述，控制在400字以内，包含 Trigger -> Action -> Result]
...
**输出语言**：\`scene_blocks/*.md\` 的所有自然语言内容使用与 "New Memories List" 中记忆相同的语言；META 字段名（created/updated/summary/heat）和 \`[DELETED]\` 等标记保持英文。
...
6. **删除文件的唯一方式**：使用 **write** 工具将文件内容写为 \`[DELETED]\` 标记（\`path\`=文件名, \`content\`=\`[DELETED]\`）。系统会自动清理带有此标记的文件。**禁止**写入空字符串。
...
1. **UPDATE（更新）【首选策略】**
   - 同一项目、模块、任务、方法、资产的补充或状态变化。
   - 可使用 **write** 整体重写，或 **edit** 局部替换。

2. **MERGE（合并）**
   - 合并后的新 block 应该是概括性更强的工作场景。
...
请参考这个模板输出 .md 文件内容，或基于已有 md 进行更新。每个 md 控制在 1500 字符内。
...
## 工作场景
[说明这个 Scene Block 适用于哪类项目、模块、任务、方法体系或协作场景]

## 适用条件
[说明这套方法在什么情况下适用]

## 核心 SOP
[这是本文件最重要的部分。沉淀可复用流程、执行步骤、协作方式或 Agent 操作规则]

## 判断逻辑
[说明团队为什么采用这些方法，背后的取舍是什么]

## 禁忌与反模式
[记录以后应避免的做法、容易误判的地方、边界条件和失败模式]

## 关键事实依据
[可为空。只保留支撑 SOP 和判断逻辑的关键事实、决策、实验结果或项目约束]
...
**触发条件**：跨场景复用的 SOP、禁忌、原则或设计方法形成稳定共识。
...
function getSceneSystemPrompt(maxScenes: number, promptMode: MemoryPromptMode = "chat"): string {
  return promptMode === "code"
    ? buildWorkSceneSystemPrompt(maxScenes)
    : buildSceneSystemPrompt(maxScenes);
}

/**
 * Build the (system, user) prompt pair for L2 scene extraction.
 *
 * Mirrors upstream `buildSceneExtractionPrompt`. The caller is expected to
 * parse the LLM response into a structured `SceneExtractionAction` list via
 * `../parsers/scene-action.js` — this port does not perform file writes.
 */
