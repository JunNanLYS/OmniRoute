/**
 * L3 persona / team-doctrine prompt family — adapted from TencentDB Agent Memory (MIT).
 *
 * Upstream source:
 *   MemoryCore/src/core/prompts/persona-generation.ts
 *     (PERSONA_SYSTEM_PROMPT, TEAM_MEMORY_SYSTEM_PROMPT, buildPersonaPrompt,
 *      PersonaPromptParams, PersonaPromptResult)
 *
 * Local adaptation:
 *   - Prompt text is preserved verbatim from upstream.
 *   - The 2000-char cap on chat persona and 1200-char cap on team doctrine
 *     are surfaced as constants so the caller can enforce / assert them.
 *   - The persona generation tool calls (write/edit against `persona.md`)
 *     are removed; the parser port returns structured content strings.
 *
...
/** Chat persona hard cap (chars). Mirrors upstream "2000 字内". */
export const PERSONA_MAX_CHARS = 2000;

/** Team doctrine hard cap (chars). Mirrors upstream "1200 字内". */
export const TEAM_DOCTRINE_MAX_CHARS = 1200;
...
export interface PersonaPromptParams {
  ...
  triggerInfo?: string;
  /** @deprecated Kept for call-site compatibility; no longer used in the prompt. */
...
请你结合已有的 persona.md 和新增/变化的 block 信息深度分析，然后输出最终 persona 内容。

## ⛔ 输出约束（必须严格遵守）

1. **最终 persona 内容以字符串形式返回**（由工程系统持久化为 \`persona.md\`）。不再使用 file tool 调用。
2. **只能操作 persona 这一个对象**，禁止引用任何其他文件（包括 scene_blocks/、.metadata/ 等）。
3. **输出必须只包含最终的 persona 文档**，不要包含思考过程、分析步骤或任何非 persona 内容。

### 严格禁止
- **禁止过长**：persona.md 内容总长度不要超过 ${PERSONA_MAX_CHARS} 字符，及时做总结和删除不重要的信息。
- **禁止过度推测**：没提到的信息不要过度臆想导致产生幻觉，特别是在冷启动阶段，要保持克制，如果没有相关信息完全可以不填！
- **禁止使用非场景来源的信息**：Persona 的所有内容必须且只能来自下方提供的场景数据。
...
请使用以下格式输出 persona 内容（只输出 Markdown 文档本身，不要包含你的解释）。可以做自主调整（信息不足时可以减少或新增 chapter）（**必须保持 Markdown 格式**）：
...
1. **最终内容以字符串形式返回**（由工程系统持久化为 \`persona.md\`）。
2. **只返回一个对象**，禁止引用任何其他文件。
3. **输出必须只包含最终 Markdown 文档**，不要包含分析过程或解释。

## 🚫 严格禁止

- **禁止超过 ${TEAM_DOCTRINE_MAX_CHARS} 字**：最终 \`persona.md\` 必须高度压缩，求精不求多。
...
请参考以下格式输出内容。可以删减章节，但必须保持 Markdown 格式，全文不超过 ${TEAM_DOCTRINE_MAX_CHARS} 字。
...
- ✅ 输出最终 Markdown 文档
- ✅ 最终内容不超过 ${TEAM_DOCTRINE_MAX_CHARS} 字
  ...
  const existingPersonaSection = existingPersona
    ? isCodeMode
      ? `\n## 📄 当前 Team Operating Doctrine（工程已预加载）\n\n` +
        `*以下是现有 persona.md 中 Team Operating Doctrine 的完整内容（${existingPersona.length} 字符）。更新后必须压缩在 ${TEAM_DOCTRINE_MAX_CHARS} 字以内：*\n\n` +
        `\`\`\`markdown\n${existingPersona}\n\`\`\`\n\n---\n`
      : `\n## 📄 当前 Persona（工程已预加载）\n\n` +
        `*以下是现有 persona.md 的完整内容（${existingPersona.length} 字符），基于此更新后请控制在${PERSONA_MAX_CHARS}字内：*\n\n` +
        `\`\`\`markdown\n${existingPersona}\n\`\`\`\n\n---\n`
    : "";
