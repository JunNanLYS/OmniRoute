...
> Model Context Protocol server with 106 tools across routing, cache, compression, memory, skills, proxy, pool, and context source operations.
>
...
**Opt-in, off by default.** The filter only runs when at least one of two environment variables is set; with neither set, all 106 tools are announced unchanged.
