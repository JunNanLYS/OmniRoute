...
| `app/(dashboard)/dashboard/memory/`                                          | Memory Studio (four-layer v4.0): `page.tsx` (layer-tab shell), `components/layers/` (L0Tab, L1Tab, L2Tab, L3Tab, DistillationSettingsTab), `hooks/` (useMemoryLayersApi)                                                                                  |
...
### `src/memory/` — Four-layer memory runtime (v4.0)

| Module                                | Purpose                                                                                                              |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `l0.ts` / `l1.ts` / `l2.ts` / `l3.ts` | Layer CRUD (L0 raw traces, L1 typed facts, L2 navigation, L3 working singleton)                                      |
| `db/`                                 | Standalone `memory.db` open + WAL (`core.ts`), four-layer service (`service.ts`), repositories, versioned migrations |
| `distillation/`                       | Background distillation worker, selector chain, permit/lock, DLQ, SQLite store                                       |
| `recall/` + `retrieval/`              | Owner-scoped recall facade and retrieval scoring for L1 dynamic recall                                               |
| `integration/`                        | Chat-pipeline seams: L0 capture, injection transformer, settings resolver, distillation queue/runtime                |
| `tencent/`                            | Selective port of TencentDB Agent Memory pure behavior modules                                                       |
| `vectorStore.ts`                      | Opt-in sqlite-vec hybrid (RRF k=60); loaded only when an embedding source is configured                              |
| `api/`                                | Handler helpers (`dependencies.ts` DI, `handlers/_lib.ts` owner/auth resolution) — see `docs/frameworks/MEMORY.md`   |
...
| `db/migrations/`          | Versioned SQL files (idempotent, transactional). `073_memory_vec.sql` adds `memory_vec_meta` + `needs_reindex` column (plan 21 — v3-era; the four-layer runtime keeps its own migrations under `src/memory/db/migrations/`). |
